...
def options():
    ...
    psa_parser.add_argument(
        "-v", "--version", dest="show_version", action="store_true", help="Shows version information"
    )
    psa_parser.add_argument(
        "--sub-list", dest="subscription_list", action="store_true", help="Shows a list of subscriptions"
    )
    psa_parser.add_argument("--info", dest="info", action="store_true", help="Shows a list of subscriptions")
    psa_parser.add_argument(
        "--domain-list", dest="domain_list", action="store_true", help="Shows a list of domains with their IP addresses"
    )
    psa_parser.add_argument(
        "--list-components", dest="show_components", action="store_true", help="Creates a list of available components"
    )
    psa_parser.add_argument(
        "--install-component", dest="component_install", type=str, help="Allows installation of available component"
    )
    psa_parser.add_argument(
        "--php-handler",
        dest="php_handler",
...
def info():
    ...
    psa_info = (
        ...
        + Color.RESET
        + "{3}"
    ).format(public_ip, login_url, plesk_version[0], str(domain_count))
    print(psa_info)
...
def component_install():
    call(["plesk", "installer", "--select-release-current", "--install-component", PSA_ARGS.component_install])
